...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap<K, M, H>& map) : HashMap(map.bucket_count(), map._hash_function)
{
...
HashMap<K, M, H>& HashMap<K, M, H>::operator= (const HashMap<K, M, H>& map) {
    if(&map == this) return *this; // 1.prevent self-assignment
    this->clear(); // 2.prevent memory leak
    _size = 0;
    _hash_function = map._hash_function;
    _buckets_array = std::vector<node*>(map.bucket_count() ,nullptr);
    for(auto it = map.begin(); it != map.end(); ++it) { // 4.Manually copy all other members
        auto [key, value] = *it; 
        this->insert({key, value});
    }
    return *this; // 5.Return a reference to *this (that was just reassigned).
}
...
HashMap<K, M, H>::HashMap(HashMap<K, M, H>&& map) { 
	...
	_buckets_array = std::move(map._buckets_array); // steal the memory
}

template <typename K, typename M, typename H>
HashMap<K, M, H>& HashMap<K, M, H>::operator= (HashMap<K, M, H>&& map) {
    if (&map == this) return *this; // prevent self-assignment
...
M HashMap<K, M, H>::at(const K& key) const {
    return static_cast<M>(const_cast<HashMap<K, M, H>*>(this)->at(key));
}
